...
public class CustomerActionBean extends AbstractActionBean {
    ...
    public StreamingResolution showCustomer() {
        ...
        if (request.getParameter("size") != null
            ...
            size = Integer.parseInt(request.getParameter("size"));//显示多少条
        }
    ...
    public Resolution updateCustomer() {
        ...
        if ("现有客户".equals(customer.getCustVal())) {
            customer.setActive(true);
        } else {
            customer.setActive(false);
        }
        if ("直销".equals(customer.getCoopType())) {
            customer.setDirect(true);
        } else {
            customer.setDirect(false);
        }
